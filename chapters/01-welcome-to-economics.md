...
These are designed to be attempted with a large language model as a thinking partner. The goal is not to get an answer — it is to test whether you actually understand the concept, by explaining it, challenging it, and rebuilding it from scratch.

**Exercise 1 — Teach the model scarcity.** Ask an LLM to explain scarcity to you. Then push back: ask it to explain the difference between scarcity and shortage, and between scarcity and poverty. Where does its explanation get fuzzy? Where is it precise? Now explain the distinction back to it in your own words and ask it to identify any errors. Write two sentences summarizing where the LLM's explanation was stronger than yours and where yours was stronger.

**Exercise 2 — Break the circular flow.** Describe the circular flow diagram to an LLM without showing it an image — just your words. Ask it to identify one thing the model explains well and one thing it cannot explain. Then ask it to suggest one feature you could add to the model to address the gap. Evaluate: does the addition actually fix the gap, or does it introduce new gaps? Write three sentences on what this exercise taught you about how models work.

**Exercise 3 — Argue both sides.** Ask an LLM to argue that specialization is unambiguously good for workers. Then ask it to argue the opposite: that specialization is harmful to workers. Which argument does it make more convincingly? What evidence would you need to resolve the disagreement? This exercise is not about which side is right — it is about learning to hold a real trade-off without collapsing it into a simple answer.

**Exercise 4 — Comparative advantage stress test.** Give an LLM this problem: "Cook A can plate 6 salads per hour or 4 entrées. Cook B can plate 2 salads per hour or 3 entrées. Who should specialize in what?" Ask it to walk through the reasoning step by step. Check the math yourself. Then ask: "What if Cook A gets sick mid-shift and Cook B has never plated an entrée before? Does comparative advantage still recommend specialization?" Evaluate whether the LLM handles the disruption case well, or whether it gives you the textbook answer without engaging with the brittleness.
...
```
I'm starting a semester-long Policy Brief project for my Principles
of Economics course. Each chapter, I'll add a section to the brief
that applies that chapter's tools to one specific contestable policy.
By the end of the book I should have a real policy brief, not a
collection of homework answers.

Help me set up the project. Ask me ONE question at a time and wait
for my answer.

1. Pick the policy. Pick ONE policy from this list (or propose a
   sharper one in the same shape):
   - A federal carbon tax of $50/ton on CO2 emissions
   - Raising the federal minimum wage to $17/hour
   - Breaking up Amazon, Google, or Meta under existing antitrust law
   - A national rent-control framework capping annual increases
   - Universal basic income of $12,000/year
   - Tariffs of 25% on Chinese imports
   - Federal student loan forgiveness of $20,000 per borrower
   - A national sugar tax of 2 cents per ounce on sweetened beverages
   - Universal single-payer healthcare in the US
   The policy must be (a) currently contested, (b) specific enough
   to model, (c) something the student has at least a draft opinion
   about (the brief will require defending a verdict).

2. The audience for the brief. Is this written for a member of
   Congress? A think-tank policy team? A general educated reader?
   The audience changes the depth and the rhetorical posture.

3. The opportunity cost frame. Every policy choice means NOT
   choosing alternatives. What are the two most plausible
   alternatives the brief should compare against (including doing
   nothing)?

4. The student's draft verdict before the analysis. Be honest
   about what you currently think. Most students arrive with an
   intuition; the brief will test it. The honest opening verdict
   is more useful than performed neutrality.

5. The single question the brief is going to answer. Phrase it
   sharply enough that the answer is "yes," "no," or "yes-with-
   conditions" — not "it's complicated."

After all five answers, write a 400–500 word "Brief Foundation"
section. It should:

- State the policy in one specific sentence.
- Name the audience.
- Frame the opportunity-cost comparison.
- State the central question the brief will answer.
- Name the student's draft verdict (with epistemic status — "I
  currently lean X, but I haven't done the analysis").
- End with three things you genuinely don't know yet that the
  semester's chapters should help you settle.

Save this as the first section of the brief in the Claude Project.
Every later chapter's prompt assumes this foundation exists.
```

---

**What this produces:** A 400–500 word "Brief Foundation" — the project's anchor. Every later chapter's analysis assumes this foundation is in the Project's context.

**How to adapt this prompt:**

- *For your own project:* The list of nine candidate policies is generic — substitute any policy that's specific, contested, and modelable. Avoid "should we have a market economy" (too broad) or "should this specific bill pass" (too narrow if the bill is about to expire).
- *For ChatGPT / Gemini:* Works as written. Build a Custom GPT instead of a Claude Project; the Brief Foundation goes in the GPT's instructions field.
- *For Claude Code:* Wrong tool for setup.
- *For a Claude Project:* Essential. The Project's project-level instructions should include the Brief Foundation by Chapter 2.
