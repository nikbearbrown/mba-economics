...
**Exercise 1 — Identify the AD shifter.** Tell an LLM about a current macroeconomic event (a tax change, a confidence shift, an exchange-rate move, a foreign recession). Ask: which AD shifter is at work, in which direction does the curve shift, and what's the predicted effect on price level and real GDP? Push for specifics.

**Exercise 2 — Identify the AS shifter.** Same exercise but for a supply-side shock (an oil price spike, a productivity boom, a major regulatory change). Push the LLM to distinguish supply-side from demand-side shocks correctly.

**Exercise 3 — Diagnose the zone.** Tell an LLM the recent U.S. unemployment rate, inflation rate, and capacity utilization. Ask: which zone of the SRAS curve is the economy operating in? What does that imply about the effect of additional demand stimulus?

**Exercise 4 — Trace stagflation.** Walk an LLM through the 1970s U.S. stagflation episode using the AD-AS framework. The OPEC oil shocks of 1973 and 1979 were negative supply shocks. The Federal Reserve's accommodating monetary policy was an attempt to counter the resulting recession with demand stimulus. The combination produced both high inflation and high unemployment. Push the LLM to draw out the diagram clearly in words.
...
**What you're building this chapter:** the AD/AS diagrammatic analysis of the policy at the macro level — what shifts, what doesn't, where the new equilibrium sits.
**Tool:** **Claude Project** "Policy Brief" — appends a section. **Claude Code** is useful here for a real AD/AS chart.
...
```
Chapter 24 of my Policy Brief project. Prior sections in this Claude
Project. Chapter 24 taught: aggregate demand (AD) — total spending
in the economy at each price level, downward-sloping; short-run
aggregate supply (SRAS) — total output firms supply at each price
level, upward-sloping in the short run because some prices are
sticky; long-run aggregate supply (LRAS) — vertical at potential
output, because in the long run wages and prices adjust;
equilibrium at the intersection of AD and SRAS; the difference
between demand-side and supply-side policies; the AS-AD model as
the workhorse for thinking about fiscal and monetary policy.

Write the brief's "AD/AS Analysis" section in 300–500 words.

1. **Identify what shifts.** Does your policy shift AD, SRAS, or
   LRAS? Or some combination? Specifically:
   - Demand-side shifts: fiscal stimulus, transfer payments,
     consumer-confidence policies.
   - Short-run supply shifts: tax-rate changes that affect
     short-run input costs (carbon tax, payroll tax cut).
   - Long-run supply shifts: anything that changes potential
     output (R&D investment, immigration, capital deepening,
     institutional reform).
   Most consequential policies shift more than one curve.

2. **The new equilibrium.** Show the equilibrium shift in words:
   "Original equilibrium at output Y0, price level P0. The policy
   shifts [curve] [direction]. New equilibrium at Y1, P1. Output
   moves [direction] by [magnitude]; price level moves [direction]
   by [magnitude]."

3. **The short-run-vs-long-run distinction.** Many policies have
   different short-run and long-run AD/AS effects. A demand-side
   stimulus moves output above potential in the short run (price
   level rises); in the long run, SRAS shifts left as wages catch
   up, and output returns to potential at a higher price level.
   A supply-side reform shifts LRAS rightward — but only after a
   transition that may produce short-run dislocation.

End with one sentence on whether your policy is best understood
as demand-side (move along LRAS), supply-side (shift LRAS), or
both. The political-economy debate often confuses the two; the
brief should be precise.
```

---

**What this produces:** A 300–500 word section with the AD/AS analysis. The diagram in words is sufficient if Claude Code isn't being used.

**How to adapt this prompt:**

- *For your own project:* UBI students: demand-side at the margin (transfer increases consumption), supply-side at the long-run margin (depending on labor-supply response). Carbon-tax students: short-run SRAS shift (raises costs), long-run LRAS effect (as the energy transition completes).
- *For ChatGPT / Gemini:* Works as written.
- *For Claude Code:* For an actual AD/AS diagram with the policy-induced shift, Claude Code can produce a working visualization.
- *For a Claude Project:* Append.

**Connection to previous chapters:** Ch 19's macro-scale identification + Ch 22's inflation analysis + Ch 24's AD/AS framework together describe the policy's macro signature.
