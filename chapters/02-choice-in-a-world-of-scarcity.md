...
**Exercise 1 — Build Alphonso's budget constraint.** Tell an LLM the setup: $10 budget, $2 burgers, $0.50 bus tickets. Ask it to derive the equation of the budget constraint, identify the slope, and explain in plain English what the slope represents. Then change one variable — raise the price of bus tickets to $1 — and ask it to redo the analysis. Verify the math by hand. Where does the LLM cut corners or skip a step? Rewrite the explanation in the way you would teach a friend.

**Exercise 2 — Find your own sunk cost.** Describe to an LLM a recent decision you made (or are making) where you suspect sunk costs are influencing your thinking — a relationship, a job, a project, a course you're considering dropping. Ask the LLM to identify the sunk cost specifically and explain what your decision *should* depend on if you ignored it. Then push back: where does the LLM's framing oversimplify your actual situation? What context does it miss?

**Exercise 3 — Curve a frontier.** Ask an LLM to explain why the PPF bows outward instead of being a straight line. Evaluate its explanation. If the explanation invokes "the law of diminishing returns" without explaining the mechanism, push it: *why* does diminishing returns happen? Keep pushing until the LLM explains it in terms of resources being differentially suited to different jobs. Notice how many follow-ups it takes to get to the real explanation.

**Exercise 4 — Marginal vs. average.** Ask an LLM the difference between marginal cost and average cost. Then give it this scenario: "You've eaten 6 slices of pizza. You're full. You feel slightly sick. Your friend offers you a 7th slice for free. Marginal cost: zero. Marginal benefit: probably negative. Average cost so far: whatever you paid divided by 6." Ask the LLM whether you should eat the 7th slice and why. Use this to test whether the LLM is reasoning about marginals or sliding back into average thinking.
...
**Project:** Policy Brief — chosen in Chapter 1.
**What you're building this chapter:** the production-possibilities and opportunity-cost frame for your policy.
**Tool:** **Claude Project** "Policy Brief — [Your Policy]" — appends the second section.
...
```
Chapter 2 of my Policy Brief project. The Brief Foundation is in
this Claude Project. Chapter 2 taught: the production possibilities
frontier (PPF) as the model of opportunity cost made visible; the
budget constraint as the individual analog; the law of increasing
opportunity cost (the curve bends because resources aren't equally
suited to every use); efficiency vs. equity as a fundamental
trade-off the PPF can't itself adjudicate.

Write the brief's "PPF and Opportunity Cost" section. Cover three
things in 300–500 words.

1. **The PPF for this policy.** What two goods (or two outcomes)
   are being traded against each other in this policy choice?
   Sketch in words where the current point sits on the PPF and
   where the policy's advocates claim it would move us. (Example:
   for a carbon tax, the PPF is "current goods" vs. "future
   environmental quality"; for minimum wage, "employment" vs.
   "wages-for-employed-workers.") Be honest about whether the
   advocates' claim is a movement TO the frontier (efficiency
   gain) or a movement ALONG the frontier (re-allocation).

2. **The opportunity cost named explicitly.** What does the policy
   force the country to give up? Most policy debates skip this
   step — they argue for the gain without naming the foregone.
   Name what's foregone in the same units (dollars, jobs,
   environmental damage, lives, time) the policy is denominated
   in.

3. **Where on the PPF you actually think we are.** Most policy
   arguments assume we're inside the PPF (where any change is a
   free lunch) or on it (where every change has cost). The honest
   reading: most economies are partially inside the PPF for some
   resources, on it for others. State your reading for the
   specific market your policy operates in.

End with one sentence naming the alternative policy that would
move toward the same goal at lower opportunity cost — even if you
don't favor it. Acknowledging the cheaper alternative is the
discipline this chapter trains.
```

---

**What this produces:** A 300–500 word section appended to the brief. The PPF framing carries through later chapters — Ch 8 (perfect competition) tests the efficiency claim; Ch 15 (poverty/inequality) tests the equity claim.

**How to adapt this prompt:**

- *For your own project:* Carbon-tax students: sketch the PPF as "current consumption" vs. "future environmental quality." Minimum-wage students: "jobs" vs. "wage levels for employed workers." Antitrust students: "consumer surplus from competition" vs. "innovation incentives from market power." Get the two axes specific.
- *For ChatGPT / Gemini:* Works as written.
- *For Claude Code:* If you want a real PPF chart with current point and policy-claimed point, Claude Code can produce a chart.js or matplotlib visualization.
- *For a Claude Project:* Append to the brief's growing document.

**Connection to previous chapters:** The Brief Foundation's opportunity-cost frame from Ch 1 gets formalized here.
